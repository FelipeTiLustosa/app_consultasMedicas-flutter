...
class AppointmentService {
  ...
  static Future<void> init() async {
    ...
    if (appointmentsData != null) {
      ...
      _appointments = decodedData.map((json) => Appointment.fromJson(json)).toList();
    }
  }
  ...
  static Future<void> _saveAppointments() async {
    final prefs = await SharedPreferences.getInstance();
    final appointmentsJson = jsonEncode(_appointments.map((a) => a.toJson()).toList());
  ...
  static Future<bool> addAttachment(String appointmentId, String filePath) async {
    final index = _appointments.indexWhere((appointment) => appointment.id == appointmentId);
    if (index != -1) {
      final currentAttachments = List<String>.from(_appointments[index].attachments);
  ...
  static Future<bool> removeAttachment(String appointmentId, String filePath) async {
    final index = _appointments.indexWhere((appointment) => appointment.id == appointmentId);
    if (index != -1) {
      final currentAttachments = List<String>.from(_appointments[index].attachments);
